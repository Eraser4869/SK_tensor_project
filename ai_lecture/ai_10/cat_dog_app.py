...
def load_model1():
    try: 
        ...
        print(current_dir + '/cat_dog_classifier.keras')
        model = tf.keras.models.load_model(current_dir + '/cat_dog_classifier.keras')
...
def load_model2():
    try:
        ...
        current_dir = os.path.dirname(os.path.abspath(__file__))
        print(current_dir + '/cat_dog_classifier.keras')
        return tf.keras.models.load_model(current_dir + '/cat_dog_classifier.keras')
    except Exception as e:
        st.error('모델을 로드할 수 없습니다. 경로를 확인해주세요.', icon="🚨")
        raise e
